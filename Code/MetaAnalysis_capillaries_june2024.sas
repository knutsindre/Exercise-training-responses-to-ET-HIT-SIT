...
%web_drop_table(IMPORT_FibreType);
...
PROC IMPORT DATAFILE=REFFILE
	DBMS=XLSX
	OUT=IMPORT_FibreType;
	Sheet="Only fibre type_prePost";
	GETNAMES=YES;
RUN;

PROC CONTENTS DATA=IMPORT_FibreType; RUN;


%web_open_table(IMPORT_FibreType);

/*Only interaction*/

ODS RTF FILE = '/home/oyvindskattebo0/MetaAnalysis june2024/Model12_Fibre_type_prePost_june24.rtf' STYLE= minimal;
options orientation=landscape;

proc GLIMMIX data=IMPORT_FibreType nobound METHOD=rspl; 
	class Study_nr Subject_group_nr prePost Training_type_new2;
	weight Type1_inverseVar;
	model Type1=Prepost*Training_type_new2/
		noint ddfm=satterthwaite solution cl alpha=0.05;
	random Intercept / type=VC subject=Subject_group_nr;
	random Study_nr;
	parms 1 1 1;
	lsmeans Prepost*Training_type_new2/ plots=meanplot OBSMARGINS /*E*/ cl pdiff=all adjust=bon stepdown adjdfe=row;
	Estimate
		"SIT post - SIT pre"
		PrePost*Training_type_new2	 	-1 0 0   
										1 0 0,						
		"HIT post - HIT pre"
		PrePost*Training_type_new2	 	0 -1 0   
										0 1 0,
		"ET post - ET pre"
		PrePost*Training_type_new2	 	0 0 -1   
										0 0 1/ADJUSt=bon stepdown cl alpha=0.05 adjdfe=row;
	Estimate
		"Change SIT vs change HIT"
		PrePost*Training_type_new2	 	-1 1 0   
										1 -1 0,						
		"Change SIT vs change ET"
		PrePost*Training_type_new2	 	-1 0 1   
										1 0 -1,
		"Change HIT vs change ET"
		PrePost*Training_type_new2	 	0 -1 1   
										0 1 -1/ADJUSt=bon stepdown cl alpha=0.05 adjdfe=row;
/*generate output data*/
output out=Model12_Fibre_type_prePost_24 pred=p resid=r;
run;
ODS RTF CLOSE;


/*********************************************************************************************************/
/*IMPORT FOR FIBER TYPE ANALYSIS*/


%web_drop_table(IMPORT_FIBERTYPE);
...
PROC IMPORT DATAFILE=REFFILE
	DBMS=XLSX
	OUT=IMPORT_FIBERTYPE;
	Sheet="Only fibre type_prePost";
	GETNAMES=YES;
RUN;

PROC CONTENTS DATA=IMPORT_FIBERTYPE; RUN;


%web_open_table(IMPORT_FIBERTYPE);

/*********************************************************************************************************/
/*FIBER TYPE ANALYSIS*/

ODS RTF FILE = '/home/oyvindskattebo0/MetaAnalysis june2024/Model12_Fibre_type_prePost.rtf' STYLE= minimal;
options orientation=landscape;

proc GLIMMIX data=IMPORT_FIBERTYPE nobound METHOD=rspl; 
	class Study_nr Subject_group_nr prePost Training_type_new2;
	weight Type1_inverseVar;
	model Type1=Prepost*Training_type_new2/
		noint ddfm=satterthwaite solution cl alpha=0.05;
	...
	Estimate
		...
		PrePost*Training_type_new2	 	0 -1 1   
										0 1 -1/ADJUSt=bon stepdown cl alpha=0.05 adjdfe=row;
/*generate output data*/
output out=Model12_Fibre_type_prePost pred=p resid=r;
run;
ODS RTF CLOSE;
